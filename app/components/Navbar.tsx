...
export default async function Navbar() {
    ...
    const navLinks = [
        { href: `/${locale}/create`, label: t("createStory"), icon: "wand" },
        { href: `/${locale}/discover`, label: t("discover"), icon: "compass" },
        { href: `/${locale}/pricing`, label: t("pricing"), icon: "credit-card" },
        { href: `/${locale}/contact`, label: t("contactUs"), icon: "mail" },
    ];
        ...
        <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-indigo-50">
            <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    {/* Logo */}
                    <Link href={`/${locale}`} className="flex items-center gap-2 cursor-pointer group">
